...
"""
16b_nj_boxes.py
NJ unrooted network with species bounding boxes — NO reticulations.
Poster style: large colored boxes per species, plain text strain labels.

Output: results_2/networks_nj/
"""
...
def nj_layout(taxa, D):
    ...
    dd  = {tl[i]: {tl[j]: D[i, j] for j in range(n) if i != j}
           for i in range(n)}
...
def draw_network(gene_label, mldist_path, lookup, species2hex,
    ...
    for t in taxa:
        sp_taxa[lookup(base_name(t))].append(t)
    ...
    for x0, y0, x1, y1 in edges:
        ax.plot([x0, x1], [y0, y1], color="#555555", linewidth=0.7, zorder=1)

    # Species bounding boxes
    PAD = 0.06
    box_info = {}
    for sp, members in sp_taxa.items():
        if sp == "Unknown" or not members:
            continue
        ...
        if not pts_x:
            continue
        x0 = min(pts_x) - PAD
        y0 = min(pts_y) - PAD * 0.8
        x1 = max(pts_x) + PAD
        y1 = max(pts_y) + PAD * 0.8
        rect = plt.Rectangle((x0, y0), x1 - x0, y1 - y0,
                              fill=True,
                              facecolor=(*hex_rgb(sp_hex), 0.10),
                              edgecolor=sp_hex, linewidth=2.0,
                              zorder=2)
        ax.add_patch(rect)
        box_info[sp] = (x0, y0, x1, y1)
    ...
    for t in taxa:
        ...
        ax.text(p[0], p[1], label,
                ...
                color="#cc0000" if is_novel else "#111111",
                zorder=5)

    # Species name labels outside boxes
    ...
    for sp, (bx0, by0, bx1, by1) in box_info.items():
        sp_hex = species2hex.get(sp, "#888888")
        parts  = sp.split()
        label  = f"S. {parts[1]}" if len(parts) >= 2 else sp
        box_cx = (bx0 + bx1) / 2
        box_cy = (by0 + by1) / 2
        dx = box_cx - cx_all
        dy = box_cy - cy_all
        if abs(dx) >= abs(dy):
            lx, ly, ha = (bx1 + 0.03, box_cy, "left") if dx >= 0 else (bx0 - 0.03, box_cy, "right")
        else:
            lx, ly, ha = (box_cx, by1 + 0.03, "center") if dy >= 0 else (box_cx, by0 - 0.03, "center")
    ...
    if show_detect and detect_methods:
        ...
        patches = []
        if "BLASTn" in methods_seen:
            patches.append(mpatches.Patch(color="#555555", label="BLASTn"))
        if "nhmmer" in methods_seen:
            patches.append(mpatches.Patch(color="#cc0000", label="nhmmer novel (italic*)"))
        if patches:
    ...
    margin = np.median(tip_r) * 0.55
    ax.set_xlim(all_x.min() - margin, all_x.max() + margin)
    ax.set_ylim(all_y.min() - margin, all_y.max() + margin)

    plt.tight_layout(pad=0.3)
    for ext in ("pdf", "png"):
        out = OUT_DIR / f"{gene_label}_nj.{ext}"
